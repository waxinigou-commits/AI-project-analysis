...
function Detail({ product, close }: { product: Product; close: () => void }) {
  return <div className="detail-backdrop" onMouseDown={close}>
    <aside className="detail" onMouseDown={e => e.stopPropagation()} aria-label="产品详情">
      ...
      <div className="detail-meta"><span>发布于 {product.launchedAt}</span><span>{product.pricing}</span><span>置信度 {product.confidence}%</span></div>
      <section><h3>一句话定位</h3><p>{product.positioning}</p></section>
      <div className="detail-grid"><section><h3>目标用户</h3><div className="tags">{product.targetUsers.map(x => <span key={x}>{x}</span>)}</div></section><section><h3>商业模式</h3><p>{product.businessModel}</p></section></div>
      ...
      <section><h3>增长机会</h3><ol className="opportunities">{product.opportunities.map((x,i) => <li key={x}><b>0{i+1}</b><span>{x}</span></li>)}</ol></section>
      <section><h3>风险判断</h3><ul className="risks">{product.risks.map(x => <li key={x}>{x}</li>)}</ul></section>
...
export default function App() {
  ...
  return <div className="app-shell">
    <header><Brand/><nav><a className="active" href="#products">产品雷达</a><a href="#trends">趋势洞察</a><a href="#about">情报源</a></nav><div className="header-actions"><span className="live"><i/> 每日更新</span><a className="github" href="https://github.com/waxinigou-commits/AI-project-analysis" target="_blank" rel="noreferrer">GitHub ↗</a><button className="menu" onClick={()=>setMenuOpen(!menuOpen)}>☰</button></div></header>
    {menuOpen && <div className="mobile-nav"><a href="#products">产品雷达</a><a href="#trends">趋势洞察</a><a href="#about">情报源</a></div>}
    <main>
      <section className="hero"><div><p className="eyebrow">AI COMMERCIAL INTELLIGENCE</p><h1>发现下一个<br/><em>商业信号</em></h1><p className="hero-copy">追踪全球 AI 新产品，用结构化商业分析，帮你更早看见市场正在发生什么。</p></div><div className="hero-side"><div className="pulse"><span>LIVE</span><div className="orb">{products.length}</div><p>已收录产品</p></div><div className="updated"><span>最后更新</span><b>今天 · 08:30</b></div></div></section>

      <section className="metrics">
        <article><div><span>本周新增</span><strong>+{Math.min(products.length, 24)}</strong></div><Sparkline values={[4,7,6,12,9,16,24]}/><small>较上周 <b>↑ 18%</b></small></article>
        <article><div><span>商业潜力均值</span><strong>{avg}</strong></div><Sparkline values={[63,68,67,71,74,72,avg]}/><small>高潜力产品 <b>{products.filter(p=>p.score>=80).length} 个</b></small></article>
        <article><div><span>热门赛道</span><strong>{catStats[0]?.name || "AI Agent"}</strong></div><div className="mini-bars">{[38,70,52,88,62,100].map((v,i)=><i key={i} style={{height:`${v}%`}}/>)}</div><small>连续 3 周增长</small></article>
        <article><div><span>覆盖数据源</span><strong>2</strong></div><div className="sources"><span>PH</span><span>WL</span></div><small>自动同步运行中</small></article>
      </section>

      <section className="trends" id="trends"><div className="section-title"><div><p className="eyebrow">MARKET PULSE</p><h2>赛道热度</h2></div><select value={period} onChange={e=>setPeriod(e.target.value)} aria-label="统计时间"><option>7 天</option><option>30 天</option><option>90 天</option></select></div><div className="trend-content"><div className="bar-chart">{catStats.map(x=><div className="bar-row" key={x.name}><span>{x.name}</span><div><i style={{width:`${(x.count/maxCat)*100}%`,background:x.color}}/><b>{x.count}</b></div></div>)}</div><div className="trend-note"><span>趋势判断</span><strong>{catStats[0]?.name || "AI Agent"}</strong><p>在过去 {period} 内发布密度最高。垂直场景、可量化 ROI 与工作流集成，是近期新品的共同商业化路径。</p><a href="#products">查看相关产品 ↓</a></div></div></section>
      ...
      <section className="radar" id="products"><div className="section-title"><div><p className="eyebrow">PRODUCT RADAR</p><h2>最新产品</h2></div><div className="view-count">{filtered.length} 个结果</div></div>
        ...
        <div className="product-list">{filtered.map((p,i)=><article className="product-card" key={p.id} onClick={()=>setSelected(p)} tabIndex={0} onKeyDown={e=>e.key==="Enter"&&setSelected(p)}><ProductLogo name={p.name} index={i}/><div className="product-main"><div className="product-title"><h3>{p.name}</h3><span>{p.source}</span></div><p>{p.tagline}</p><div className="tags"><span>{p.category}</span>{p.targetUsers.slice(0,2).map(x=><span key={x}>{x}</span>)}</div></div><div className="business"><span>商业模式</span><b>{p.pricingModel}</b><small>{p.pricing}</small></div><div className="signals"><span>增长信号</span>{p.signals.slice(0,2).map(x=><small key={x}>↗ {x}</small>)}</div><ScoreRing score={p.score}/><button className="arrow" aria-label={`查看 ${p.name}`}>→</button></article>)}</div>
